...
int main(){
    ...
    MeshFoliage grass = lum.loadFoliage("", 6, 10);
    MeshLiquid  water = lum.loadLiquid(69, 42);
    MeshVolumetric smoke = lum.loadVolumetric(1, .5, {});
    ...
    while(not lum.should_close){
        ...
        lum.drawMesh(tank_body, tank_body_trans);
        lum.drawMesh(tank_head, tank_head_trans);
        lum.drawMesh(tank_rf_leg, tank_rf_leg_trans);
        lum.drawMesh(tank_lf_leg, tank_lf_leg_trans);
        lum.drawMesh(tank_rb_leg, tank_rb_leg_trans);
        lum.drawMesh(tank_lb_leg, tank_lb_leg_trans);
