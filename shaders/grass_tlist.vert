...
void main() {
    ...
    //MEASURED - no difference. As expected.
    int blade_id = gl_InstanceIndex;
    int blade_vertex_id = gl_VertexIndex;
    ...
    vec2 rel2tile_shift = (vec2(blade_x, blade_y) / vec2(pco.size)) * 128.0; //for visibility
    // vec3 rel2tile = rel2world + vec3(rel2tile_shift,0);

    float rand_rot = rand(rel2tile_shift);
    rotate_blade_vert(rand_rot, rel2world, normal);
